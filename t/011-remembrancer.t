...
sub TestNonExistingKeyFetch {
    ...
    ok( !$R->load( [ "bar" ] ),
        "Attempting to load non-existing array key (1 element)" );
    ok( !$R->load( [ "foo", "bar" ] ),
        "Attempting to load non-existing array key (2 elements)" );
}

sub TestPayloadKeyStore {
    my ( $R ) = @_;
    my @baselines = (
        [ "Store scalar with no payload", [ "foo" => 0 ], ],
    ...
    for my $pair ( @baselines ) {
        ...
        for ( my $i = 0; $i < @$test_key; $i++ ) {
            ok( $test_key->[ $i ] eq $got_key->[ $i ],
                "  Retrieved key part matched original key: "
                    . $test_key->[ $i ]
            );
        }
...
sub TestKeyExistence {
    my ( $R ) = @_;
    my @baselines = (
        [ "Exists with scalar key",     [ "foo" ], ],
        [ "Exists 2 element array key", [ [ "baz", "bar" ], ] ],
    );
    ...
    ok( !defined $R->updated_at( [ "foo", "bar" ] ),
        "Non-existing key does not have an updated timestamp"
    );
}
...
sub TestPayloadRetrieval {
    ...
    my @baselines = (
        [ "Retrieve scalar with no payload", [ "foo" => 1 ], ],
    ...
    ok( !$R->load( [ 'nonsuch' ] ),
        "Non-existing key returned a false value" );
}

sub TestKeyRemoval {
    my ( $R ) = @_;
    my @baselines = (
        [ "Remove scalar key",          [ "foo" ], ],
        [ "Remove 2 element array key", [ [ "baz", "foo" ], ] ],
    );
