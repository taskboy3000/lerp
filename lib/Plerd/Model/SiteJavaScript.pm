...
package Plerd::Model::SiteJavaScript;
...
sub _build_template_file {
    my ($self) = @_;
    Path::Class::File->new($self->config->template_directory, "site_js.tt");
}
...
sub _build_uri {
    ...
    return URI->new($self->config->base_uri . "js/" . $self->publication_file->basename);
}
