...
#------------
has author_email =>
    ( is => 'rw', lazy => 1, builder => '_build_author_email' );
...
has config_template_dir => (
    is      => 'ro',
    default => sub {
        Path::Class::Dir->new( $ENV{ PLERD_HOME },
            "lib", "Plerd", "Template" );
    }
);
...
has 'datetime_formatter' =>
    ( is => 'ro', default => sub { DateTime::Format::W3CDTF->new } );

has 'engine_name' => ( is => 'ro', default => sub { "Taskboy Plerd" } );
has 'engine_uri'  => (
    is      => 'ro',
    default => sub { URI->new( 'https://github.com/taskboy3000/plerd' ) }
);
has 'engine_version' => ( is => 'ro', default => sub { $VERSION } );

has 'jumbotron_image' =>
    ( is => 'rw', predicate => 1, coerce => \&_coerce_image );
...
sub initialize {
    ...
    my $src_template_dir = Path::Class::Dir->new( $ENV{ PLERD_HOME },
        "lib", "Plerd", "Template" );
    while ( my $src_file = $src_template_dir->next ) {
        next if substr( $src_file, -3, 3 ) ne '.tt';

        my $dst_file = Path::Class::File->new( $self->template_directory,
            $src_file->basename );
        if ( -e $dst_file ) {
        ...
        copy( $src_file, $dst_file )
            or die( "Cannot copy $src_file -> $dst_file: $!" );
    }
...
sub serialize {
    my ( $self ) = @_;
    my $template_file = Path::Class::File->new( $self->config_template_dir,
        "config_file.tt" );

    my $out;
    unless (
        $self->config_tt->process(
            $template_file->stringify, { config => $self }, \$out
        )
        )
    {
