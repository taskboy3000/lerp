...
sub publish_note {
    ...
    if ( !$must_republish ) {
        ...
        if ($published_file_exists
            && ( $note->publication_file->stat->mtime >=
                $note->source_file->stat->mtime )
            )
        {
            if ( $opts{ verbose } ) {
                say "Declining to reprocess unchanged "
...
sub publish_tags_index_page {
    ...
    if ($self->_publish(
            ...
            { tag_links => $tag_links, thisURI => $tags_index->uri, }, "tags"
        )
        )
    {
        if ( $opts{ verbose } ) {
...
sub publish_json_feed {
    ...
    my $max_posts = $self->config->show_max_posts;
    my @latest_keys =
        sort { $b->[ 0 ] cmp $a->[ 0 ] } @{ $post_memory->keys };
    ...
    if ($self->_publish(
            $feed->template_file, $feed->publication_file, $vars
        )
        )
    {
        if ( $opts{ verbose } ) {
...
sub publish_front_page {
    ...
    if ($self->_publish(
            $feed->template_file, $feed->publication_file,
            { posts => \@posts, thisURI => $feed->uri }, "blog"
        )
        )
    {
        if ( $opts{ verbose } ) {
...
sub publish_archive_page {
    ...
    @posts = sort {
        $a->publication_file->basename cmp $b->publication_file->basename
    } @posts;

    if ($self->_publish(
            $feed->template_file, $feed->publication_file,
            { posts => \@posts, thisURI => $feed->uri }, "archive"
        )
        )
    {
        if ( $opts{ verbose } ) {
...
sub publish_site_css_page {
    ...
    if ( $self->_publish( $feed->template_file, $feed->publication_file, {}, )
        )
    {
        if ( $opts{ verbose } ) {
            say "Published " . $feed->publication_file->basename;
...
sub publish_all {
    ...
    if ( !$opts{ only_notes } ) {
        if ( $opts{ verbose } ) {
            say "Looking for new post files in "
        ...
        if ( -e $self->config->source_directory ) {
                ...
                )
            {
                ...
                if ( $self->should_publish_post( $post ) || $opts{ force } ) {
                    ...
                    if ( $opts{ verbose } ) {
                        say "Declining to reprocess old source: "
    ...
    if ( !$opts{ only_posts } ) {
        if ( $opts{ verbose } ) {
            say "Looking for new notes files in "
                . $self->config->source_notes_directory;
        }
...
sub forget_post {
    ...
    for my $tag ( @{ $rec->{ tags } || [] } ) {
        $self->tags_index->remove_tag_from_post( $tag,
            $rec->{ source_file } );
    }

    my $file =
        Path::Class::File->new( $self->config->publication_directory, @$key );
...
sub should_publish_post {
    ...
    if ( $post->source_file->stat->mtime >
        $post->publication_file->stat->mtime )
    {
        return 1;
    }
...
sub should_publish_note {
    ...
    if ( $record->{ publication_file } ) {
        my $pub_file =
            Path::Class::File->new( $record->{ publication_file } );
        if ( !-e $pub_file ) {
...
sub _publish {
    ...
    unless ( $self->publisher->process( $tmpl_fh, $vars, $trg_fh, ) ) {
        die(sprintf(
                "assert[processing %s] %s\n",
                $target_file, $self->publisher->error()
            )
        );
    }
